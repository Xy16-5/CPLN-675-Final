...
ggplot() +
   geom_sf(data=MSA_fishnet01,aes(fill=as.factor(developed_2011)),color='transparent')+
...
ggplot() +
  geom_sf(data=MSA_fishnet01, aes(fill=as.factor(developed_2001)),color='transparent') +
...
```
```{r final dataset}
colnames(MSA_fishnet01)[which (names(MSA_fishnet01)=="developed_2011")] <-"lc_change"
```
```{r dataset fishnet}
...
F.score_table[which.max(F.score_table$F.score),] #0.0049; but we don't want their balance

testSetProbs <- 
  testSetProbs %>% 
  mutate(
        predClass_005 = as.factor(ifelse(testSetProbs$probs >= 0.005 ,1,0)),
...
ConfusionMatrix.metrics <-
  dat %>%
    mutate(probs = predict(Model6, dat, type="response") ,
           Threshold_0.5_Pct = as.factor(ifelse(probs >= 0.005 ,1,0)),
           Threshold_3_Pct =  as.factor(ifelse(probs >= 0.03, 1,0))) %>%
    mutate(TrueP_0.5 = ifelse(lc_change  == 1 & Threshold_0.5_Pct == 1, 1,0),
           TrueN_0.5 = ifelse(lc_change  == 0 & Threshold_0.5_Pct == 0, 1,0),
