...
knitr::opts_chunk$set(echo = FALSE,message=FALSE,warning=FALSE,include=FALSE,error=T)
options(scipen=99)
```
...
![Pittsburgh MSA Conuties](Img/Pittsburgh MSA.png)
...
predsForMap <-         
  dat %>%
    ...
    dplyr::select(lc_change,Threshold_1_Pct,Threshold_2_Pct) %>%
...
```{r dat2 process}
dat11_pred <- dat_infill %>% 
  dplyr::select(predict_2021.infill)

dat2_demand <- st_join(dat2, dat11_pred)

dat2_demand <- dat2_demand %>%
...
```{r demand county plot, include=T, error=T}
county_specific_metrics %>%
  gather(Variable, Value, -NAME) %>%
...
## Supply-side Change
[] We tried building a model with 2000-2010 data to project population change with distance to highway, but the result coefficient is insignificant and positive, which suggests that the closer to highway, the smaller the population growth. Since it conflicts with normal assumption, for the purpose of this project, we assume that a 1% increase in transportation infrastructure stock will lead to 0.2% increase in population^[Duranton, G., & Turner, M. A. (2012). Urban Growth and Transportation.The Review of Economic Studies, 79 (4), 1407-1440.
http://dx.doi.org/10.1093/restud/rds010].

```{r pop and highway model}
popProject <- glm(pop_Change ~ wetlands_2001 + forest_2001  + farm_2001 + otherUndeveloped_2001 + lagDevelopment + distance_highways,
  data = dat)

```

```{r pop and high way, include=T, result='asis'}
stargazer(popProject, type='html')
```

[] project new pop
1% (0.01) increase in road = 0.2% increase in pop
1.22% (1.22) increase in road = (1.22*0.002) increase in pop
...
```{r pop proj with highway, include=T}
countyPopulation_2021_withHighway %>%
  gather(Variable,Value, -NAME) %>%
  ggplot(aes(reorder(NAME,-Value),Value)) +
  geom_bar(aes(fill=Variable), stat = "identity", position=position_dodge()) +
...
```{r supply demand plot, include=T}
...
```{r allegheny allocation, include=T}
Allegheny <-
  dat_infill_withHighway %>%
...
```{r armstrong allocation, include=T}
Washington <-
  dat_infill_withHighway %>%
