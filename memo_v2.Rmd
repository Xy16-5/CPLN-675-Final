...
F.score_table[which.max(F.score_table$F.score),] #0.01;
...
Armstrong <-
  dat_name_demand %>%
  filter(NAME == "Armstrong")

Armstrong_landUse <- rbind(
  filter(Armstrong, forest_2011 == 1 | wetlands_2011 == 1 ) %>%
  dplyr::select() %>% mutate(Land_Use = "Not Suitable"),
  filter(Armstrong, developed_2011 == 1) %>%
  dplyr::select() %>% mutate(Land_Use = "Developed"))

grid.arrange(
ggplot() +
  geom_sf(data=Armstrong, aes(fill=factor(ntile(Development_Demand,5))), colour=NA) +
  geom_point(data=Armstrong_landUse, aes(x=xyC(Armstrong_landUse)[,1], 
                                        y=xyC(Armstrong_landUse)[,2], colour=Land_Use),
                                        shape = 18, size = 1) +
  geom_sf(data=st_intersection(PittsburghHighways,filter(Pittsburgh_MSA, NAME=="Armstrong")), color="#6c757d", size=1) +
  scale_fill_manual(values = palette5, name="Development\nDemand",
                    labels=substr(quintileBreaks(Armstrong,"Development_Demand"),1,5)) +
  scale_colour_manual(values = c("#9163cb","#e29578")) + 
  labs(title = "Development Potential, 2021: Armstrong") + mapTheme() +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)),

ggplot() +
  geom_sf(data=Armstrong, aes(fill=factor(ntile(pop_Change,5))), colour=NA) +
  geom_point(data=Armstrong_landUse, aes(x=xyC(Armstrong_landUse)[,1], 
                                        y=xyC(Armstrong_landUse)[,2], colour=Land_Use),
                                        shape = 18, size = 1) +
  geom_sf(data=st_intersection(PittsburghHighways,filter(Pittsburgh_MSA, NAME=="Armstrong")), color="#6c757d", size=1) +
  scale_fill_manual(values = palette5, name="Population\nChange",
                    labels=substr(quintileBreaks(Armstrong,"pop_Change"),1,5)) +
  scale_colour_manual(values = c("#9163cb","#e29578")) + 
  labs(title = "Projected Population, 2021: Armstrong") + mapTheme() +
  guides(fill = guide_legend(order = 1), colour = guide_legend(order = 2)), ncol=2)
```
